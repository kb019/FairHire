...
- [x] 2. Authentication service - backend
  - [x] 2.1 Implement HR user registration endpoint `POST /api/auth/register/hr`
  ...
  - [x] 2.2 Implement applicant registration endpoint `POST /api/auth/register/applicant`
  ...
  - [x] 2.3 Implement shared login endpoint `POST /api/auth/login`
  ...
  - [x] 2.4 Implement JWT auth middleware `backend/src/middleware/auth.ts`
  ...
  - [x] 2.5 Write unit tests for auth validators and JWT middleware
...
- [x] 3. Job posting CRUD - backend
  - [x] 3.1 Implement job posting endpoints in `backend/src/routes/jobPostings.ts`
  ...
  - [x] 3.2 Write property test for HR data isolation (Property 6)
  ...
  - [x] 3.3 Implement score history recording
  ...
  - [x] 3.4 Write unit tests for job posting ownership enforcement
...
- [x] 5. Analyzer service - job posting bias detection
  - [x] 5.1 Create `backend/src/analyzer/jobPostingAnalyzer.ts`
  ...
  - [x] 5.2 Implement compliance score calculation in `backend/src/analyzer/scoreCalculator.ts`
    - Formula: `Math.max(0, 100 - (critical * 15) - (warning * 5) - (suggestion * 1))`
    - _Requirements: 4.6, 10.1_

  - [x] 5.3 Write property test for compliance score bounds (Property 4)
  ...
  - [x] 5.4 Implement stateless analysis endpoint `POST /api/analyze/job-posting`
    - Accept `{ text: string }`; run `jobPostingAnalyzer`; return issues + score
    - No database writes; used for real-time feedback
    - _Requirements: 4.1, 4.9_

  - [x] 5.5 Wire analysis into job posting save/update flow
  ...
  - [x] 5.6 Write unit tests for bias detection rules
    - Test each issue type with known-flagged and known-clean phrases
    - Test EEO statement detection (present vs. absent)
    - _Requirements: 4.2, 4.3, 4.4, 4.5_

- [x] 6. Issue acknowledgement and educational content
  - [x] 6.1 Add `acknowledged` and `acknowledgement_note` fields to Issue objects stored in `ethics_reports.issues` JSONB
  ...
  - [x] 6.2 Add `explanation`, `suggestion`, and `educational_link` fields to each Issue in the analyzer output
    - Map each issue type to a static educational resource URL (EEOC guidelines, etc.)
    - _Requirements: 11.1, 11.2, 11.3_

  - [x] 6.3 Write unit tests for acknowledgement flow
    - Test that acknowledged issues are excluded from score recalculation
    - Test that missing justification returns 400
    - _Requirements: 11.4_

- [x] 7. Resume parsing pipeline - backend
  - [x] 7.1 Create `backend/src/analyzer/resumeParser.ts`
  ...
  - [x] 7.2 Create `backend/src/analyzer/resumePrinter.ts`
    - Serialize a `ParsedResume` back to a canonical plain-text string
    - _Requirements: 6.3_

  - [x] 7.3 Write property test for resume round-trip parsing (Property 1)
  ...
  - [x] 7.4 Write unit tests for resume parser
...
- [x] 8. Redactor - identity signal removal
  - [x] 8.1 Create `backend/src/analyzer/redactor.ts`
  ...
  - [x] 8.2 Write property test for redaction completeness (Property 2)
  ...
  - [x] 8.3 Write unit tests for redactor
...
- [x] 9. Resume submission endpoint - backend
  - [x] 9.1 Implement `POST /api/resumes` in `backend/src/routes/resumes.ts`
  ...
  - [x] 9.2 Write property test for Anonymous ID stability (Property 3)
  ...
  - [x] 9.3 Write integration tests for resume submission endpoint
...
- [x] 11. Candidate review and contact disclosure - backend
  - [x] 11.1 Implement candidate review endpoints in `backend/src/routes/candidates.ts`
  ...
  - [x] 11.2 Implement contact disclosure endpoint `POST /api/job-postings/:id/candidates/:anonymousId/request-contact`
  ...
  - [x] 11.3 Write property test for contact disclosure isolation (Property 5)
  ...
  - [x] 11.4 Write unit tests for candidate review authorization
...
- [x] 12. Analytics endpoints - backend
  - [x] 12.1 Implement analytics endpoints in `backend/src/routes/analytics.ts`
  ...
  - [x] 12.2 Write unit tests for analytics aggregation
    - Test empty-state responses (no job postings)
    - Test correct ranking of top issue types
    - _Requirements: 12.1, 12.3, 12.4_

- [x] 13. Ethics report PDF export - backend
...
- [x] 15. Frontend - authentication screens
  - [x] 15.1 Create `frontend/src/pages/RegisterHR.tsx` and `RegisterApplicant.tsx`
  ...
  - [x] 15.2 Create `frontend/src/pages/Login.tsx`
  ...
  - [x] 15.3 Write unit tests for auth form validation
    - Test password policy enforcement in the UI
    - Test error message display on failed login
    - _Requirements: 1.2, 1.4, 2.2, 2.4_

- [x] 16. Frontend - HR job posting editor with real-time analysis
  - [x] 16.1 Create `frontend/src/pages/JobPostingEditor.tsx`
  ...
  - [x] 16.2 Implement inline issue highlighting in the editor
  ...
  - [x] 16.3 Implement compliance score indicator
  ...
  - [x] 16.4 Implement issue acknowledgement UI
...
- [x] 18. Frontend - anonymous candidate review
  - [x] 18.1 Create `frontend/src/pages/CandidateList.tsx`
  ...
  - [x] 18.2 Create `frontend/src/pages/CandidateDetail.tsx`
...
- [x] 20. Frontend - HR analytics dashboard
...
- [x] 22. Wiring, routing, and authorization guards
  - [x] 22.1 Set up React Router routes in `frontend/src/App.tsx`
  ...
  - [x] 22.2 Add Express route registration and global error handler in `backend/src/app.ts`
  ...
  - [x] 22.3 Write integration tests for full HR workflow
  ...
  - [x] 22.4 Write integration tests for full applicant workflow
